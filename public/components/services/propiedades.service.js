...
var publicAPI = {
  ...
  function _setPropiedad(pPropiedad){
      var propiedadLista = _getPropiedad();
      propiedadLista.push(pPropiedad);
      localStorage.setItem('lspropiedadLista', JSON.stringify(propiedadLista));
    }

    function _getPropiedad(){
      var propiedadLista = JSON.parse(localStorage.getItem('lspropiedadLista'));
      if(propiedadLista == null){
      localStorage.setItem('lspropiedadLista', JSON.stringify(jugadores));
      propiedadLista = jugadores;
      }
      return propiedadLista;
    }

  function _updatePropiedad(pPropiedadMod){

      var propiedadLista = _getPropiedad();

      for(var i = 0; i < propiedadLista.length; i++){

         if(propiedadLista[i].codigo === pPropiedadMod.codigo){
          propiedadLista[i] = pPropiedadMod;
        }
      }
